...
beforeEach(function() {
  nav.goTo(nav.pages.multipleWithValidation);
});

describe('key-value-editor validation', function() {

  describe('when provided a string to create a regex for validation', function() {
  ...
  describe('when provided a validation regex', function() {
    it('should validate the key with the regex', function() {
  ...
  describe('when provided a pseudo-regex validation object', function() {
    it('should validate the key with the object .test() function', function() {
      var editor = kve.findAll().get(2);
      var first = kve.getFirstKeyInput(editor);
      first.clear().sendKeys('ab');
      // should be valid, no help blocks
      expect(kve.getHelpBlock(editor, 'validation error').count()).toEqual(0);
